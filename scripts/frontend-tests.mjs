...
const client=await readFile("lib/genlayer/client.ts","utf8"); for(const token of ["TransactionStatus.FINALIZED","ExecutionResult.FINISHED_WITH_RETURN","connect(\"studionet\")","estimateTransactionFeesForWrite","feeValue","canonicalCheck"]) if(!client.includes(token)) throw new Error(`Client missing ${token}`);
const amounts=await readFile("lib/genlayer/amounts.ts","utf8"); if(!amounts.includes("10n ** 18n")||!amounts.includes("genToWei")) throw new Error("Missing explicit GEN-to-wei conversion");
