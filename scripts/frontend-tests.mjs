...
const client=await readFile("lib/genlayer/client.ts","utf8"); for(const token of ["TransactionStatus.FINALIZED","ExecutionResult.FINISHED_WITH_RETURN","connect(\"studionet\")","canonicalCheck"]) if(!client.includes(token)) throw new Error(`Client missing ${token}`);
