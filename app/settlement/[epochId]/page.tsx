...
export default function Settlement() {
  ...
  const load = async () => {
    try {
      ...
      if (account) {
        setCredit(BigInt(String(await readContract(config.pool, "get_funder_credit", [id, account]))));
        setRefund(await readContract(config.pool, "get_refund_settlement", [id, account]));
      }
  ...
  return <div className="mx-auto max-w-6xl px-6 pb-24 pt-14">
    ...
    {error ? <p className="mt-10 border-l-4 border-[var(--coral)] p-4">{error}</p> : !epoch || !pool ? <p className="mt-10 mono">Reading pool and epoch state…</p> : <>
      ...
      <div className="mt-10 flex flex-wrap gap-4">
        ...
        <div className="plate p-5 text-sm">Connected funder credit: {formatGen(credit)}<br />Refund settlement: {refund?.status || "NONE"}{refund?.identity && <><br />Identity: {refund.identity}</>}</div>
      </div>
