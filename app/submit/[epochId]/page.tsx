...
export default function Submit() {
  const {epochId} = useParams<{epochId: string}>(); const [stage, setStage] = useState<TxStage>(); const [error, setError] = useState(""); const [claimId, setClaimId] = useState<number>(); const [hash, setHash] = useState(""); const {address:account,ensureWriteReady}=useWallet();
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      setError(""); const {address:writer,provider} = await ensureWriteReady(); const epoch = Number(epochId); const before: any = await readContract(config.rounds, "get_epoch", [epoch]);
      const result = await writeAndConfirm(writeClient(writer, provider), config.rounds, "submit_claim", [epoch, parsed.data.title, parsed.data.type, parsed.data.repo, parsed.data.primary, parsed.data.secondary, parsed.data.corroboration], 0n, setStage, async () => {
