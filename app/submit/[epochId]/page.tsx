...
export default function Submit() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      setError(""); const {address:account,provider} = await connectWallet(); const epoch = Number(epochId); const before: any = await readContract(config.rounds, "get_epoch", [epoch]);
      const result = await writeAndConfirm(writeClient(account, provider), config.rounds, "submit_claim", [epoch, parsed.data.title, parsed.data.type, parsed.data.repo, parsed.data.primary, parsed.data.secondary, parsed.data.corroboration], 0n, setStage, async () => {
