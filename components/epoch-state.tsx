...
export function EpochState({ id }: { id: number }) {
  ...
  const actionArea = () => {
    ...
    if ((epoch.status === "CLAIMS_OPEN" || epoch.status === "EVALUATING") && beforeClose) return <><p className="mt-3 max-w-xl text-sm leading-6">Funding is open while this round accepts claims. Contributors may submit completed work before the claims deadline.</p><div className="mt-5 flex flex-wrap gap-3"><ContractAction contract="pool" method="fund" args={[id]} value={genToWei(1)} label="Fund pool with 1 GEN" onComplete={confirmFund} /><Link className="button primary" href={`/submit/${id}`}>Submit a claim</Link></div></>;
